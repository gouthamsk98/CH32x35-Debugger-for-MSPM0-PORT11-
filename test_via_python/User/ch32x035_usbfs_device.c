 ...
 * @return  none
 */
uint8_t USBFS_Endp_DataUp(uint8_t *pbuf, uint16_t len)
{
 
  if( USBFS_Endp_Busy[ DEF_UEP2 ] == 0 )
  {
    /* copy mode */
    memcpy(USBFSD_UEP_BUF(DEF_UEP2), pbuf, len );

    /* tx length */
    USBFSD_UEP_TLEN(DEF_UEP2) = len;
    /* response ack */
    USBFSD_UEP_TX_CTRL(DEF_UEP2) = (USBFSD_UEP_TX_CTRL(DEF_UEP2) & ~USBFS_UEP_T_RES_MASK ) | USBFS_UEP_T_RES_ACK;
    /* Set end-point busy */
...
/*********************************************************************
 * @fn      USBFS_endp_dataRead
 *
 * @brief   USBFS device data read
 *
 * @return  length
 */
uint8_t USBFS_endp_dataRead(uint8_t *pbuf)
{
  uint16_t rx_len = 0;
  if(len > 0)
  {
    rx_len = len;
    memcpy(pbuf, USBFS_EP1_Buf, len);
    // memset(USBFS_EP1_Buf, '\0', sizeof(USBFS_EP1_Buf));
    len = 0;
  }
  else 
  {
